  ...
  const getFilms = async (month, day, year = constants.FESTIVAL_YEAR) => {
    ...
    const films = await page.evaluate((m, d, y) => {
      ...
      return Array.from(document.querySelectorAll('li.block-type-film'))
        .filter(film => {
          return showTypes.includes(film.getAttribute('data-showtype'));
        })
        .map(film => {
    ...
    // console.log(films.filter(film => film.category === 'short-film'));

    // console.log(Array.from(new Set(films.map(film => film.title))));
